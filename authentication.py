...
def handle_register(self, json_data):
    ...
    self.server.db.create_account(username, salt, pass_hash)
    result["success"] = True
    result["info"] = "Account created"
    auth_token = hashlib.md5(
        (salt + username + str(time.time())).encode('utf-8')).hexdigest()
    self.server.db.auth.authenticate(
        self.client_address[0], username, auth_token)
...
def handle_login(self, json_data):
    ...
    salt = self.server.db.get_account_salt(username)
    if not salt:
        result["success"] = False
...
class Authenticator:
    ...
    def __init__(self):
        ...
        self.timeout_auth_thread = threading.Thread(
            target=self.timeout_auth_tokens, daemon=True)
